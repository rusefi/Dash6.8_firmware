...
#include "FreeRTOS.h"
#include  "semphr.h"
SemaphoreHandle_t nmeaLockMutex = NULL;
...
inline void nmea_callback(nmea_t *nmea)
{
  ...
  {
        // Сбрасываем флаги ошибок USART
        LL_USART_ClearFlag_ORE(nmea->usart); // Сброс флага переполнения приема (Overrun Error)
        LL_USART_ClearFlag_NE(nmea->usart);  // Сброс флага ошибки четности (Noise Error)
        LL_USART_ClearFlag_FE(nmea->usart);  // Сброс флага ошибки формата (Framing Error)
    }
}
//###########################################################################################################################
bool nmea_init(nmea_t *nmea, USART_TypeDef *usart, uint16_t buf_size)
{
	...
	nmeaLockMutex = xSemaphoreCreateMutex();
	return true;
}
//###########################################################################################################################
void nmea_loop(nmea_t *nmea)
{
	if (xSemaphoreTake(nmeaLockMutex, portMAX_DELAY) == pdTRUE) {
...
int get_utc_offset(float latitude, float longitude) {
    // Пример таблицы таймзон для России
	typedef struct {
    float lat_min;
    float lat_max;
    float lon_min;
    float lon_max;
    int utc_offset; // Смещение UTC в часах
} TimezoneEntry;
    TimezoneEntry timezone_table[] = {
        {41.1851f, 81.8587f, 19.6389f, 55.0f, 3},  // Москва и центральная Россия (UTC+3)
        {41.1851f, 81.8587f, 55.0f, 60.0f, 4},    // Самара (UTC+4)
        {41.1851f, 81.8587f, 60.0f, 70.0f, 5},    // Екатеринбург (UTC+5)
        {41.1851f, 81.8587f, 70.0f, 80.0f, 6},    // Омск (UTC+6)
        {41.1851f, 81.8587f, 80.0f, 90.0f, 7},    // Красноярск (UTC+7)
        {41.1851f, 81.8587f, 90.0f, 100.0f, 8},   // Иркутск (UTC+8)
        {41.1851f, 81.8587f, 100.0f, 110.0f, 9},  // Якутск (UTC+9)
        {41.1851f, 81.8587f, 110.0f, 120.0f, 10}, // Владивосток (UTC+10)
        {41.1851f, 81.8587f, 120.0f, 180.0f, 12}, // Анадырь (UTC+12)
        // Добавьте другие записи по необходимости
    };

    size_t table_size = sizeof(timezone_table) / sizeof(TimezoneEntry);
    for (size_t i = 0; i < table_size; i++) {
        if (latitude >= timezone_table[i].lat_min && latitude <= timezone_table[i].lat_max &&
            longitude >= timezone_table[i].lon_min && longitude <= timezone_table[i].lon_max) {
            return timezone_table[i].utc_offset;
        }
    }
    return 0; // Возвращаем 0, если координаты не попадают в известные таймзоны
}
//###########################################################################################################################
